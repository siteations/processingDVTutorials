...
void setup( ) {
  size(600, 600);
  collection= loadJSONArray("http://data.cityofnewyork.us/resource/kzmz-ivhb.json");
  loadData( );
...
void loadData( ) {
  for (int i = 0; i < collection.size(); i++) {
    ...
    //"primary_disposal_facility" : "Covanta - Essex",
    //"district" : "MN01",

    String district = item.getString("district");
    String disFacility = item.getString("primary_disposal_facility");


    addEdge(district, disFacility);
  }
  /*
    addEdge("joe", "dog");
   addEdge("joe", "tea");
